...
import {
  Title, Text, SimpleGrid, Paper, Group, Stack, RingProgress, Center,
  ...
  return (
    <Stack gap="lg">
      <div>
        ...
        <Text c="dimmed" size="sm">Search API & MCP Gateway</Text>
      </div>

      <SimpleGrid cols={{ base: 1, sm: 3 }}>
        {loading ? [1,2,3].map(i => <Skeleton key={i} height={140} radius="lg" />) :
          [
            { title: "Total Requests", value: (stats?.total_requests ?? 0).toLocaleString(), icon: IconArrowUpRight, idx: 0 },
            { title: "Credits Used", value: (stats?.total_credits ?? 0).toLocaleString(), icon: IconFlame, idx: 1 },
            { title: "Avg Response", value: `${stats?.avg_response_time_ms ?? 0}ms`, icon: IconClock, idx: 2 },
          ].map(({ title, value, icon: Icon, idx }) => (
            <Paper key={title} p="lg" radius="lg" style={{
              background: gradients[idx], color: "white", position: "relative", overflow: "hidden", transition: "transform 0.2s",
            }}
            ...
            {stats && stats.by_endpoint.length > 0 ? (
              <Table>
                <Table.Thead><Table.Tr>
                  <Table.Th>Endpoint</Table.Th><Table.Th ta="right">Requests</Table.Th><Table.Th ta="right">Credits</Table.Th>
